...
const bugres = ' 𝐃𝐨𝐧𝐞 𝐬𝐞𝐧𝐭 𝐛𝐮𝐠 𝐛𝐲 𝐕𝐚𝐦𝐩𝐢𝐫𝐞 𝐤𝐢𝐥𝐥𝐞𝐫 𝐕𝟕☯︎'
...
if (isCmd) {
console.log(chalk.white.bgRed.bold('Ada Pesan, Om'), color(`[ 𝐆𝐢𝐝𝐝𝐲 𝐓𝐞𝐧𝐧𝐨𝐫 ]`, `green`), color(`FROM`, `red`), color(`${pushname}`, `red`), color(`Text :`, `yellow`), color(`${body}`, `blue`))
}
...
║▬▭▬▭▬▭▬▭▬▭
│ 𝐎𝐰𝐧𝐞𝐫 : ${author}
║ 𝐂𝐫𝐞𝐚𝐭𝐨𝐫 : 𝐆𝐢𝐝𝐝𝐲 𝐓𝐞𝐧𝐧𝐨𝐫\`\〽️
│ 𝐎𝐰𝐧 𝐍𝐨 : 𝟎𝟏𝟎𝟒𝟐𝟒𝟓𝟔𝟓𝟗
║ 𝐏𝐫𝐞𝐟𝐢𝐱 : 𝐌𝐔𝐋𝐓𝐈
│▬▭「 𝐕𝐚𝐦𝐩𝐢𝐫𝐞 𝐊𝐢𝐥𝐥𝐞𝐫 𝐂𝐥𝐢𝐞𝐧𝐭 」▭▬
║› 𝐆𝐢𝐝𝐝𝐲 𝐓𝐞𝐧𝐧𝐨𝐫 ©Copyright
╰━━━━━━━━━━━━━━━━━━━⬣`
let sections = [{
title: 'Choose Menu',
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Vampire Killer\",\"url\":\"https://youtube.com/@revancrash\",\"merchant_url\":\"https://youtube.com/@revancrash\"}"
}]
}) 
})} 
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Giddy Tennor\",\"url\":\"https://youtube.com/@revancrash\",\"merchant_url\":\"https://youtube.com/@revancrash\"}"
}]
}) 
})} 
}}, {userJid: m.sender, quoted: m}) 
...
await loading()
const version = require("baileys/package.json").version
const bugmenu = `╭──(   𝐕𝐚𝐦𝐩𝐢𝐫𝐞         )
║ᨒ \`\〽️
│ 𝐍𝐚𝐦𝐞 : ${m.pushName}
║▬▭▬▭▬▭▬▭▬▭
│ 𝐎𝐰𝐧𝐞𝐫 : ${author}
║ 𝐂𝐫𝐞𝐚𝐭𝐨𝐫 : 𝐑𝐞𝐯𝐚𝐧𝐂𝐫𝐚𝐬𝐡\`\〽️
│ 𝐎𝐰𝐧 𝐍𝐨 : 0104245659
║ 𝐏𝐫𝐞𝐟𝐢𝐱 : 𝐌𝐔𝐋𝐓𝐈
│▬▭「 𝐕𝐚𝐦𝐩𝐢𝐫𝐞 𝐤𝐢𝐥𝐥𝐞𝐫 」▭▬
║› 𝐆𝐢𝐝𝐝𝐲 𝐓𝐞𝐧𝐧𝐨𝐫 ©Copyright
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Giddy Tennor\",\"url\":\"https://youtube.com/@revancrash\",\"merchant_url\":\"https://youtube.com/@revancrash𝐑𝐞𝐯𝐚𝐧𝐂𝐫𝐚𝐬𝐡\"}"
}]
}) 
})} 
}}, {userJid: m.sender, quoted: m}) 
...
case 'vampirebug': case 'vampiregas': 
if (!isPremium) return reply(mess.only.premium)
...
case 'tennorkill': case 'tennorcrash': 
if (!isPremium) return reply(mess.only.premium)
if (!q) return reply(`Example: ${prefix + command} 254×××`)
target = q.replace(/[^0-9]/g,'')+"@s.whatsapp.net"
reply(bugres)
...
case "bugoflow": {
				...
				if (!isPremium) return reply(mess.premium)
				const sections = []
				const listgroup = await zyn.getAllGroups(true)
				if (!isNaN(parseInt(text)) && text.endsWith("@g.us")) {
					...
					reply("𝙎𝙪𝙘𝙘𝙚𝙨 𝙎𝙚𝙣𝙙 𝘽𝙪𝙜")
				} else if (input.length > 0) {
					const failed = []
					await reply(mess.bugrespon)
					for (const x of input) {
						if ((await zyn.onWhatsApp(x)).length > 0 && !owner.includes(x.split("@")[0]) && !kontributor.includes(x.split("@")[0]) && x.split("@")[0] !== Creator && x !== botNumber) {
							for (let i = 0; i < 1; i++) {
					...
					if (input.filter((x) => !failed.includes(x)).length > 0 && input.filter((x) => failed.includes(x)).length > 0) {
						await reply("𝙎𝙪𝙘𝙘𝙚𝙨 𝙎𝙚𝙣𝙙 𝘽𝙪𝙜 𝙏𝙤 " + input.filter((x) => !failed.includes(x)).map((x) => "@" + x.split("@")[0]).join(" ") + "\n\n𝙁𝙖𝙞𝙡𝙚𝙙 𝙎𝙚𝙣𝙙 𝘽𝙪𝙜 𝙏𝙤 " + input.filter((x) => failed.includes(x)).map((x) => "@" + x.split("@")[0]).join(" "))
					} else if (input.filter((x) => !failed.includes(x)).length > 0 && input.filter((x) => failed.includes(x)).length == 0) {
						await reply("𝙎𝙪𝙘𝙘𝙚𝙨 𝙎𝙚𝙣𝙙 𝘽𝙪𝙜 𝙏𝙤 " + input.filter((x) => !failed.includes(x)).map((x) => "@" + x.split("@")[0]).join(" "))
					} else if (input.filter((x) => !failed.includes(x)).length == 0 && input.filter((x) => failed.includes(x)).length > 0) {
						await reply("𝙁𝙖𝙞𝙡𝙚𝙙 𝙎𝙚𝙣𝙙 𝘽𝙪𝙜 𝙏𝙤 " + input.filter((x) => failed.includes(x)).map((x) => "@" + x.split("@")[0]).join(" "))
					}
					...
					reply(`Example : ${prefix + command} 120###@g.us`)
				}
			}
			break
			case "gcui": {
